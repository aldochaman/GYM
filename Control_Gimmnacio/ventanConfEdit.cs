            ...
            else
            {
                ...
                if (MessageBox.Show("¿Desea guardar?", "Guardar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (lbTitulo.Text == "Productos Registrados")
                    {
                        ...
                        string q12 = "select cantidad as s from product where idProduc = '" + txt1.Text.Trim() + "'";
                        ...
                        qry = "Update product set nomProduc ='" + txt2.Text.Trim() + "',cantidad=" + cant + ",precio=" +Convert.ToDecimal( txt3.Text.Trim()) + " where idProduc = " + txt1.Text.Trim() + "";
                        if (dts.update(qry) == true)
                        {
                            MessageBox.Show("Elemento Guardado", "Exito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            limpiar();
                            q2 = "select idProduc as Id,nomProduc as Producto,cantidad as Cantidad,precio as Precio from product";
                            consultar(q2);
                            bloqueaEliminar(2);
                        }
