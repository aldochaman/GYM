...
namespace Control_Gimmnacio
{
    public partial class reportes : Form
    {
        ...
        public void llenarReporte()
        {
            ...
            cbTipoReporte.Items.Add("Membresias");
            cbTipoReporte.Items.Add("Historial de Membresias");
            cbTipoReporte.Items.Add("Visitantes");
            cbTipoReporte.Items.Add("Visitantes Historial");
        ...
        private void cbTipoReporte_SelectedIndexChanged(object sender, EventArgs e)
        {
               ...
               if (cbTipoReporte.SelectedItem.ToString()=="Socios")
            {
                ...
                q = "Select idSocio as Clave, nombre as Nombre, CONVERT(VARCHAR,fNacimiento,103) as [Fecha de Nacimiento], sexo as Sexo,dir as Direccion, tel as [Telefono de Contacto], correo as Correo  from socio";
                dataGridView1.DataSource = dts.consulta(q).Tables[0];
               // btnRes.Visible = false;

            }
            if (cbTipoReporte.SelectedItem.ToString()== "Membresias")
            {
                q = "select M.idMemS as [Clave Socio/Membresia],(select S.nombre as Nombre from socio S where S.idSocio=idMems) Socio, M.idControl as [ID Membresia],M.tMem as Tipo,CONVERT(VARCHAR,M.fechaIngreso,103)  as Inicio,CONVERT(VARCHAR,M.fechatermino,103) as Termino, M.prom as Promocion, M.estado as Estatus from memSocio M  order by Socio";
                dataGridView1.DataSource = dts.consulta(q).Tables[0];
               // btnRes.Visible = false;
            }
            if(cbTipoReporte.SelectedItem.ToString()== "Visitantes")
            {
                q = "  select v.cantidad as Cantidad,CASE WHEN Socio IS  NULL THEN Visitante ELSE Socio END AS Visita ,precio,CONVERT(VARCHAR,v.fecha,103) as [Fecha de ingreso] from visitante v";
                dataGridView1.DataSource = dts.consulta(q).Tables[0];
                //btnRes.Visible = true;
            }
            if (cbTipoReporte.SelectedItem.ToString() == "Visitantes Historial")
            {
                pickfechaini.Visible = true;
                pickfechafinal.Visible = true;
                pickfechaini.Value = DateTime.Now;
                pickfechafinal.Value = DateTime.Now;
                StringBuilder query = new StringBuilder();
                query.Append("select v.cantidad as Cantidad,CASE WHEN Socio IS  NULL THEN Visitante ELSE Socio END AS Visita ,precio,CONVERT(VARCHAR,v.fecha,103) as [Fecha de ingreso] from visitante_historial v");
                query.AppendFormat("  where v.fecha >= '{0}' AND v.fecha <= '{1}'", pickfechaini.Value.ToString("yyyy-MM-dd"), pickfechafinal.Value.ToString("yyyy-MM-dd"));
                //dataGridView1.DataSource = dts.consulta(query.ToString()).Tables[0];
                //btnRes.Visible = true;
            }
            if (cbTipoReporte.SelectedItem.ToString()=="Productos")
            {
                q = "select idProduc as Id,nomProduc as Producto,cantidad as Cantidad,precio as Precio from product";
                dataGridView1.DataSource = dts.consulta(q).Tables[0];
                dataGridView1.Columns["Precio"].DefaultCellStyle.Format = "N2";
                dataGridView1.Columns["Precio"].DefaultCellStyle.NullValue = "0.00";
                //btnRes.Visible = false;
            }
            if (cbTipoReporte.SelectedItem.ToString()== "Historial de Membresias")
            {
            ...
            {
                q = "select idventas as [Codigo del Producto], nomProduc as [Nombre del Producto], cantidad as [Cantidad], total as [Total],CONVERT(VARCHAR,fecha,103) as [Fecha de la Venta] from ventas";
                dataGridView1.DataSource = dts.consulta(q).Tables[0];
            }
        }
        string h = "",f="";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
          ...
          private void pickfechaini_ValueChanged(object sender, EventArgs e)
          {
               StringBuilder query = new StringBuilder();
               query.Append("select v.cantidad as Cantidad,CASE WHEN Socio IS  NULL THEN Visitante ELSE Socio END AS Visita ,precio,CONVERT(VARCHAR,v.fecha,103) as [Fecha de ingreso] from visitante_historial v");
               query.AppendFormat("  where v.fecha >= '{0}' AND v.fecha <= '{1}'", pickfechaini.Value.ToString("yyyy-MM-dd"), pickfechafinal.Value.ToString("yyyy-MM-dd"));
          ...
          private void pickfechafinal_ValueChanged(object sender, EventArgs e)
          {
               StringBuilder query = new StringBuilder();
               query.Append("select v.cantidad as Cantidad,CASE WHEN Socio IS  NULL THEN Visitante ELSE Socio END AS Visita ,precio,CONVERT(VARCHAR,v.fecha,103) as [Fecha de ingreso] from visitante_historial v");
               query.AppendFormat("  where v.fecha >= '{0}' AND v.fecha <= '{1}'", pickfechaini.Value.ToString("yyyy-MM-dd"), pickfechafinal.Value.ToString("yyyy-MM-dd"));
