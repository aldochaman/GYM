...
{
    public partial class vRutinaUs : Form
    {
        string fHoy = "";
        public vRutinaUs()
        {
        ...
        private void vRutinaUs_Load(object sender, EventArgs e)
        {
        ...
        private void vRutinaUs_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
        ...
        public void consultaSocio2(string c)
        {
            ...
            foreach (DataRow row in dt2.Rows)
            {
                ...
                txt_curlacostado_series.Text = ExtraeColumna(row["curl_acostado"].ToString(),1);
                txt_curlacostado_rep.Text = ExtraeColumna(row["curl_acostado"].ToString(), 2);
                txt_curlacostado_super.Text = ExtraeColumna(row["curl_acostado"].ToString(), 3);
            }

        }

          private string ExtraeColumna(string Ps_row, int Ps_Posicion)
          {
               // Dividir la cadena en partes usando la coma como delimitador
               string[] partes = Ps_row.Split(',');

               // Verificar si la posición es válida
               if (Ps_Posicion < 1 || Ps_Posicion > partes.Length)
               {
                    throw new ArgumentException("La posición proporcionada no es válida");
               }

               // Obtener el valor en la posición especificada
               string valorEnPosicion = partes[Ps_Posicion - 1];
