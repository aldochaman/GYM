...
namespace Control_Gimmnacio
{
    public partial class confVentana : Form
    {
        public confVentana()
        {
          ...
          //private void llenacombosocios()
          //{
          //     /*compruebo que los usuarios no esten ya repetidos*/
          //     DataSet dataC = new DataSet();
          //     string qCompruena = "Select idsocio + ' - ' + nombre as Socio from Socio";
          //     dataC = dts.consulta(qCompruena);
          //     DataTable dt2 = dataC.Tables[0];
          //     if (dt2 != null)
          //     {
          //          // Itera sobre las filas del DataTable y agrega cada valor al ComboBox
          //          foreach (DataRow row in dt2.Rows)
          //          {
          //               // Asumiendo que la columna que quieres mostrar es la primera (índice 0)
        ...
        private void btnAMem_Click(object sender, EventArgs e)
        {
            ...
            if (textNomMem.Text == "" || txtPMem.Text == ""||numDiasMem.Value ==0)
            {
            ...
            else
            {
                ...
                {
                    string qry1 = "Insert into memb (nomMem,tipopersona, precioMem, dias) values('" + textNomMem.Text + "' , '" + txtTipo.Text + "'," + Convert.ToDouble(txtPMem.Text.Trim()) + ","+numDiasMem.Value+")";
